...
const profileFilmSchema = z.object({
  id: z.string().min(1),
  title: z.string().min(1),
  director: z.string(),
...
const profileDataBaseSchema = z.object({
  ...
  filmography: z.array(profileFilmSchema).optional(),
  outsideAgency: z.array(profileFilmSchema).optional(),
  ...
  .superRefine((data, ctx) => {
    if (data.loginType === "normal" && !data.password) {
      ctx.addIssue({
        ...
        path: ["password"],
        message: "Password ÃƒÂ© obrigatÃƒÂ³ria para loginType 'normal'.",
      });
    }
  });
